...
- [ ] 2.6-pre-delay Add the N-sample compensation delay line primitive (§2.4b inserts these; legacy has only `audio_delay_one_sample`/`block_delay`, no settable-length delay): preallocated ring buffer state, `ModuleKind`, dispatch arm, DSP, and kernel declaration with latency = its delay length
