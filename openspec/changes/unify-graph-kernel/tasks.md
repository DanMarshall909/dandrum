...
- [ ] 2.6-pre Add the two primitives a lowered kernel graph needs but the legacy engine lacks: an N-sample compensation delay line (§2.4b inserts these; legacy has only `audio_delay_one_sample`/`block_delay`) and `control_to_audio` (§2.5 inserts these; legacy `SignalType::is_compatible_with` rejects control→audio). Both need DSP + `ModuleKind` + dispatch arms
- [ ] 2.6 Rewire `preparation.rs` to lower the flattened kernel graph into the legacy `Graph` that `compiled_patch::compile` already consumes, keeping `compiled_patch.rs`/`render_plan.rs`/`graph_processor` untouched and offline/realtime parity tests green. **Depends on 3.1 (builtin ports) and 2.6-pre (missing primitives).** Bridge also maps `port_defaults: f64` → `CompiledNode.parameters: String`, sets all nodes `ExecutionScope::Global` until `poly` (§4), and resolves `audio_output_index` from root ports (§3.4)
