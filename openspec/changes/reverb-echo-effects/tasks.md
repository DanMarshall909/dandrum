...
- [ ] 1.1 Create `src/rust-engine/src/delay_line.rs` with `DelayLine` struct: circular buffer, write head, read with fractional interpolation
- [ ] 1.2 Implement `write(sample: f32)` and `read(delay_samples: f32) -> f32` with linear interpolation
- [ ] 1.3 Implement cubic (4-point) interpolation mode switchable per instance
- [ ] 1.4 Implement modulation offset input for read position
- [ ] 1.5 Implement `reset()` method (zero buffer, reset write head)
- [ ] 1.6 Write unit tests: integer delay, fractional delay, modulation, interpolation modes, reset, power-of-two buffer sizing
- [ ] 1.7 Add `pub mod delay_line` to `src/rust-engine/src/lib.rs`

## 2. One-Pole Filter Primitive

- [ ] 2.1 Add `OnePoleFilter` struct to `src/rust-engine/src/filter.rs` implementing `FilterAlgorithm`: one-pole low-pass with configurable cutoff, `process(input: f32) -> f32`
- [ ] 2.2 Add `OnePoleMode` enum with `Lowpass` and `Highpass` variants
- [ ] 2.3 Implement `set_cutoff(hz: f64, sample_rate: f64)` method using standard one-pole coefficient formula `a = exp(-2*pi*cutoff/sample_rate)`
- [ ] 2.4 Implement `reset()` (zero internal state)
- [ ] 2.5 Write unit tests: DC gain, impulse response, frequency roll-off, highpass attenuation, cutoff sweep, no NaN at extreme cutoff
- [ ] 2.6 Add `use` exports if needed so `crate::filter::OnePoleFilter` is accessible

## 3. Echo Module (Pipeline Composition)

- [ ] 3.1 Create `src/rust-engine/src/echo.rs` with `Echo` struct containing stereo pair of: `DelayLine` + `OnePoleFilter` (damping) + feedback gain stage
- [ ] 3.2 Implement stereo process function: per-channel pipeline of `DelayLine::write` → `DelayLine::read` → `OnePoleFilter::process` → feedback gain → mix with dry input
- [ ] 3.3 Implement ping-pong mode (cross-channel feedback routing: left delay output feeds right delay input and vice versa)
- [ ] 3.4 Implement tempo sync mode: compute delay time from BPM + note division
- [ ] 3.5 Implement wet/dry mix output
- [ ] 3.6 Implement parameter setters: delay time (ms), feedback (0–0.99), damping cutoff, wet/dry, ping-pong toggle, sync division
- [ ] 3.7 Write unit tests: impulse response, feedback decay, ping-pong routing, damping filter effect, tempo sync calculation, wet/dry mix, edge cases (zero delay, max feedback)
- [ ] 3.8 Add `pub mod echo` to `src/rust-engine/src/lib.rs`
