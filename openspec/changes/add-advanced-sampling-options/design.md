...
- Provide reusable sampling primitives suitable for drum-machine and instrument patches.
- Support prepared sample assets with regions, slices, loops, root note, gain, pan, and playback metadata.
- Support sample maps with key ranges, velocity ranges, round-robin/probability alternates, and deterministic selection.
- Support one-shot, gated, looped, reversed, pitched, sliced, and choke-group playback.
- Keep all decoding, validation, metadata preparation, resampling for preparation, and allocation off the audio thread.
...
- Implement a full DAW sampler, Kontakt clone, SFZ importer, or sample editor.
- Implement disk streaming in the first slice.
...
- `sample_player` — plays a prepared sample region as one-shot, gated, or looped audio.
- `sample_map_player` — selects a region/zone from a prepared sample map by note, velocity, round-robin, probability, and seed.
- `sample_slicer` — plays one slice from a prepared slice table, suitable for chopped breaks and rhythmic one-shots.
- `voice_choke` or explicit choke-group support inside the voice/sample playback layer — stops or fades previous voices in the same exclusive group.
...
```text
note event + velocity
   -> sample_map_player selects prepared region/zone
   -> sample_player voice renders region playback
   -> envelope/filter/modulation/effects modules shape the result
   -> audio_output
```

This keeps sample playback reusable for drum hits, chromatic instruments, sliced loops, and synthetic sample sources.

### Make selection deterministic

Round-robin and probability selection must be deterministic for identical render inputs. Each prepared player instance should own seeded selection state. Re-rendering the same event stream with the same seed should produce the same selected zones.
...
### Prefer preloaded samples for v1

V1 should preload decoded sample buffers into memory during preparation. Disk streaming is deliberately out of scope because it complicates realtime safety, plugin portability, failure modes, and test determinism.

Large sample libraries can be addressed later with explicit streaming primitives and buffering contracts.
...
- `selected_region` (`control`, optional diagnostic/future extension).
...
- `max_voices`, `voice_steal`, `choke_group`, `choke_mode`.
...
- `sync_mode` — `free` for v1; host/tempo sync is a future extension unless already available.
...
- Whether sample-map selection should be a standalone control/event module or bundled into `sample_map_player` initially. Prefer bundling for v1 unless the graph/event model makes separate selection cheap.
