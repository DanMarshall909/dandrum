...
Full workstation sampling, creative/granular/time-stretch sampling, and DJ-style sample streaming are separate specs.

## What Changes

- Add an `advanced-sampling-options` capability covering prepared sample assets, sample regions, sample maps, zone selection, velocity layers, round-robin alternates, choke groups, explicit slicing, modest looping, and deterministic playback behaviour.
...
- Build around small primitives rather than a monolithic sampler module:
  - `sample_region`/prepared region metadata for bounded file-backed audio ranges.
  - `sample_player` for one-shot, gated, looped, reversed, and pitched playback of a prepared sample region.
  ...
  - `sample_slicer` for triggerable playback of explicit slices from a prepared break/loop asset.
...
- Keep expensive work off the audio thread: file IO, decoding, preparation-time resampling where needed, validation, region indexing, explicit slice metadata import, and buffer allocation happen before rendering.
...
- `advanced-sampling-options`: Defines prepared sample assets, sample regions, sample maps, zone selection, velocity layers, round-robin alternates, explicit slicing, modest looping, choke groups, and realtime-safe playback requirements.

### Modified Capabilities

- `built-in-modules`: The built-in module registry will include the sample playback primitives and any required selector/voice/choke helpers.
