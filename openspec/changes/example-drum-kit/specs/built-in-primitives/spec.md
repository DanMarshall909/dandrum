...
The `delay_line` module SHALL write incoming audio to an internal circular buffer and read it back after a configurable
number of samples. It SHALL support fractional delay via linear interpolation.
...
The `envelope_follower` module SHALL accept an `audio_in` audio input and output a control signal tracking the signal
amplitude with configurable `attack` and `release` time constants.
