...
The `multiply` module SHALL accept two control inputs (`a` and `b`) and produce a single control output (`product`)
where `product[i] = a[i] × b[i]` for each sample `i`.
...
The multiply module SHALL be registered in the built-in module registry, have a corresponding `ModuleKind::Multiply`
variant, and be dispatchable at render time.
