...
- [ ] 2.1 Define a small realtime event type and bounded queue in the Rust engine layer.
- [ ] 2.2 Add non-blocking event submission APIs that return accepted or dropped status.
- [ ] 2.3 Drain queued events at the start of realtime audio rendering without blocking.
- [ ] 2.4 Expose realtime event submission status through the C FFI.

## 3. JUCE Callback Integration

- [ ] 3.1 Update `RustEngineSource` so `getNextAudioBlock` does not acquire the shared engine lock.
- [ ] 3.2 Update MIDI note handling to enqueue events instead of mutating the engine under the shared lock.
- [ ] 3.3 Remove callback-time console logging from MIDI input handling.
