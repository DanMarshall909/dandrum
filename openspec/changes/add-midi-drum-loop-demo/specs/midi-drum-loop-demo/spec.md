...
The system SHALL provide a user-invoked integration command that loads a drum container patch or preset and plays a repeating drum loop by sending MIDI note events through the existing engine playback path.
...
- **THEN** the system SHALL load the configured drum container patch or preset before starting the loop
...
- **THEN** it SHALL exercise drum container loading, MIDI event scheduling, engine event submission, realtime render preparation, and audio rendering through the existing wrapper path
...
- **WHEN** the built-in drum loop events are rendered against the selected drum container patch or preset in tests
...
- **THEN** it SHALL cover drum container asset loading, loop event scheduling, engine preparation, MIDI event submission, and rendered audio output
- **THEN** it SHALL NOT require a physical or virtual audio device to pass

### Requirement: Drum container is the default demo asset

The drum loop demo SHALL use a drum container asset as its default loaded instrument rather than loading the raw drum-kit patch directly.

#### Scenario: Default asset is a drum container

- **WHEN** the user invokes the drum loop demo without an explicit asset override
- **THEN** the system SHALL select a drum container patch or preset as the default asset
