...
- [x] 1.1 Add or identify test coverage proving the drum-machine patch or preset loads, validates, and renders non-empty audio from kick, snare, and hat MIDI note events.
- [x] 1.2 Add failing integration coverage for the built-in drum loop event schedule proving it loads the selected drum-machine asset, prepares the engine, submits MIDI events, and produces non-empty rendered output.

## 2. Drum Loop Scheduling

- [x] 2.1 Define the fixed four-beat drum loop pattern with kick, snare, and hat note events using notes supported by the drum-machine patch or preset.
...
- [x] 3.1 Add a `dandrum-drum-machine-demo` integration demo option that loads the drum-machine patch or preset and starts the MIDI drum loop.
