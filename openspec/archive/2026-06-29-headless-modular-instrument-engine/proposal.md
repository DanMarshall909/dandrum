...
The project needs a clear product and engineering contract for an OSS virtual instrument that starts headless, with
routing and scripting as first-class capabilities rather than GUI-driven afterthoughts. Defining the modular engine
first keeps the future GUI, plugin, and library ecosystem grounded in a testable core.
...
- Make VCA/control routing first-class so any compatible VCA/control output can connect to any compatible VCA/control
  input.
...
- `script-modules`: Script modules as graph nodes with custom ports, state, event/control processing, and bounded
  scheduling.
