...
The compiled offline render path SHALL correctly process voice-scoped modules that route audio or control signals to
global-scoped modules. Voice-scoped modules SHALL be processed first so their outputs are available when global-scoped
modules execute.

#### Scenario: Voice oscillator through global mixer produces correct output

- **WHEN** a graph has a voice-scoped `oscillator` connected to a global `audio_mixer` connected to a global
  `audio_output`
