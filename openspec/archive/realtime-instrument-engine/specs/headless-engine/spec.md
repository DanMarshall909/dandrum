...
The engine SHALL process patches in blocks using a shared scheduling model for offline rendering and realtime instrument
rendering.
...
- **THEN** the graph SHALL advance through the same block scheduling semantics used by offline rendering for event
  ordering and module state progression
