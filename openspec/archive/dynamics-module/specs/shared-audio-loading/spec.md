...
The engine SHALL provide a `audio_loading` module that encapsulates PCM WAV file loading and asset preparation logic,
reusable by the sampler, convolution, and any future audio-file-consuming modules.
...
- **THEN** the function SHALL load all assets referenced by modules matching the filter and return a map of module ID to
  loaded audio data

### Requirement: Sampler delegates to shared loading

The existing sampler module SHALL delegate to `audio_loading` for WAV loading and asset preparation, retaining its
public API through re-exports for backward compatibility.
