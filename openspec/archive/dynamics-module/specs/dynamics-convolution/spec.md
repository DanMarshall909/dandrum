...
The engine SHALL provide a built-in `convolution` module with `audio_in` (Audio) input, `audio_out` (Audio) output, and
a `wet` (Control) input for dry/wet mix. The module SHALL load an impulse response from a declared asset (kind:
`impulse_response`) and perform FFT-based partitioned convolution.
...
The module SHALL implement partitioned convolution using the overlap-add method, processing input audio against the
loaded IR in fixed-size partitions.
